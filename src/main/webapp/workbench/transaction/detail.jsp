...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	for(int i =0;i<dvlist.size() ; i++){
		if("0".equals(pMap.get(dvlist.get(i).getValue()))){
			point = i;
			break;
		}
	}
...
<script type="text/javascript">
		...
		$.ajax({
			...
			success:function (resp) {
				...
				$.each(resp,function (index,elemt) {
					html +='<tr>';
					html +='<td>'+elemt.stage+'</td>';
					html +='<td>'+elemt.money+'</td>';
					html +='<td>'+elemt.possibility+'</td>';
					html +='<td>'+elemt.expectedDate+'</td>';
					html +='<td>'+elemt.createTime+'</td>';
					html +='<td>'+elemt.createBy+'</td>';
					html +='</tr>';
				})
				$("#HistoryTran").html(html);
			}
		})
	}
	function changeStage(stage,i) {
		$.ajax({
			...
			success:function (resp) {
				if(resp.success){
					$("#stage").html(resp.t.stage);
					$("#possibility").html(resp.t.possibility);
					$("#editBy").html(resp.t.editBy);
					$("#editTime").html(resp.t.editDate);
					changeIcon(stage,i);
					showHistoryList();
				}else {
					alert("变动失败");
	...
	function changeIcon(stage,i) {
		var currnetStage = stage;
		var currentPossiblity = $("#possibility").html();
		var index = i;
		var point = <%=point%>;
		if(currentPossiblity == 0){
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			if("0".equals(currentPossiblity)){
				for (int i = 0 ; i < dvlist.size() ; i++){
					DicValue dv = dvlist.get(i);
					String listStage = dv.getValue();
					String listPossiblity = pMap.get(listStage);
					if("0".equals(listPossiblity)){
						if(currentStage.equals(listStage)){
						%>
				<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-remove mystage" data-toggle="popover"
		...
		-----------
						...
						%>
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-record mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dv.getText()%>" style="color: #000000;"></span>
		-----------
					<%
					}
				...
				-----------
						...
						if(i == index){
								%>
				<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage" data-toggle="popover"
					  data-placement="bottom" data-content="<%=dv.getText()%>" style="color: #90F790;"></span>
				-----------
				<%
						}else if(i < index){
				%>
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover"
			  data-placement="bottom" data-content="<%=dv.getText()%>" style="color: #90F790;"></span>
		-----------
				<%
						}else if(i > index){
